...
var isSkippedURL = function isSkippedURL(skipKeys) {
  ...
  if (!!document.referrer) {
    var lowerReferrer = decodeURIComponent(document.referrer.toLowerCase()).split("?");

    if (!!lowerReferrer.length && lowerReferrer[1]) {
      var referrerSearch = lowerReferrer[1];
      var skipKey = skipKeys.find(function (key) {
        return referrerSearch.includes(key.toLowerCase());
      });
...
(function () {
  var generateOneLinkURL = function generateOneLinkURL() {
    ...
    var mediaSource = _parameters$afParamet.mediaSource,
        ...
        _parameters$referrerS = parameters.referrerSkipKeys,
        referrerSkipKeys = _parameters$referrerS === void 0 ? [] : _parameters$referrerS;
    ...
    if (isSkippedURL(referrerSkipKeys)) {
      return null;
