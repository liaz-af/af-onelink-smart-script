/**
 * AF Smart Script (Build 2.3.1-poc)
 */
...
var getQueryParamsAndSaveToSessionStorage = function getQueryParamsAndSaveToSessionStorage(websiteUrl) {
  ...
  //if queryParams is empty? push?

  var savedQueryParams = JSON.parse(sessionStorage.getItem('ss_incoming_params')) || [];
  savedQueryParams.unshift(queryParams); // we used unshift becuse the order matter

  sessionStorage.setItem('ss_incoming_params', JSON.stringify(savedQueryParams));
};
...
function getCurrentURLParams(aggregateValues) {
  ...
  if (Object.keys(sessionStorage).includes('ss_incoming_params')) {
    var incomingParamsFromSS = JSON.parse(sessionStorage['ss_incoming_params']);
    console.log('🚀 ~ file: smartScript.js:77 ~ getCurrentURLParams ~ aggregateValues', aggregateValues);
    console.log('🚀 ~ file: smartScript.js:80 ~ getCurrentURLParams ~ incomingParamsFromSS', incomingParamsFromSS);
    ...
    if (currentURLParams) {
      console.log('Object with one of the values found:', currentURLParams);
    } else {
      console.log('No object with one of the values found.');
    }
  } else {
    console.log("Key 'ss_incoming_params' not found in sessionStorage.");
  }
...
var version = "2.3.1-poc";

var formatVersion = version.replace(/\./g, '_'); //replace . with _

QRCode();
getQueryParamsAndSaveToSessionStorage(window.location.href);
